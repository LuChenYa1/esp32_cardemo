...
//     // 4. 设置灰度传感器校准参数
//     gray_sensor_set_calibration(GRAY_SENSOR_LEFT, LEFT_WHITE_VALUE, LEFT_BLACK_VALUE);
//     gray_sensor_set_calibration(GRAY_SENSOR_RIGHT, RIGHT_WHITE_VALUE, RIGHT_BLACK_VALUE);
//     ESP_LOGI(TAG, "灰度传感器校准参数已设置");
...
//     ESP_LOGI(TAG, "系统启动完成！");
//     ESP_LOGI(TAG, "========================================");
//     ESP_LOGI(TAG, "Timer 0 (1ms): 灰度扫描 + 转弯检测");
//     ESP_LOGI(TAG, "Timer 1 (10ms): PD控制 + 转弯状态机");
//     ESP_LOGI(TAG, "ADC采样任务: 1ms周期");
//     ESP_LOGI(TAG, "数码管显示任务: 5秒轮播");
//     ESP_LOGI(TAG, "舵机控制任务: 3秒往复");
//     ESP_LOGI(TAG, "========================================");
...
//         // 检查定时器是否正常工作
//         // Timer 0应该每5秒触发约5000次（1ms周期）
//         // Timer 1应该每5秒触发约500次（10ms周期）
//         if (timer0_delta < 4500 || timer0_delta > 5500) {
//             ESP_LOGW(TAG, "警告：Timer 0中断频率异常（期望5000次/5s，实际%lu次/5s）", timer0_delta);
//         }
